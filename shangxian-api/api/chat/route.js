...
export async function POST(request) {
  try {
    const { message, conversationId, toolOutputs } = await request.json();
    
    // 如果提供了工具输出，则处理工具调用响应
    if (toolOutputs) {
      return NextResponse.json(await handleToolOutputs(toolOutputs));
    }
    
    // 否则，发起新对话
    return NextResponse.json(await startNewChat(message, conversationId));
  } catch (error) {
